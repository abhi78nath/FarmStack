...
function ProfilePage() {
  ...
  const fetdata = async () => {
    let user = JSON.parse(localStorage.getItem("userInfo"));
    const res = await axios.get(`https://farmstack-server-4.onrender.com/api/blog/post/${user._id}`);
    const data = res.data;
  ...
  const fetchallblogs = async () => {
    const res = await axios.get(`https://farmstack-server-4.onrender.com/api/blog/`);
  ...
  const fetuser = async () => {
    let user = JSON.parse(localStorage.getItem("userInfo"));
    const res = await axios.get(`https://farmstack-server-4.onrender.com/api/user/getuser/${user._id}`);
    const data = res.data;

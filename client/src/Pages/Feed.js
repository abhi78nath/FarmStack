...
function Feed() {
  ...
  const getFeeds = async () => {
    ...
    //API call
    const response = await fetch("https://farmstack-server-4.onrender.com/api/feed/fetchallfeed", {
      method: "GET",
      headers,
    });
  ...
  const getUsers = async () => {
    ...
    //   headers.Authorization = `Bearer ${token}`;
    // }
    const response = await fetch("https://farmstack-server-4.onrender.com/api/user", {
      method: "GET",
      headers,
    });
    const json = await response.json();
